...
const QUICK_QUESTIONS = [
  { label: '用纏論分析 AAPL', strategy: 'chan_theory' },
  { label: '波浪理論看 TSLA', strategy: 'wave_theory' },
  { label: '分析 NVDA 趨勢', strategy: 'bull_trend' },
  { label: '箱體震盪策略看 MSFT', strategy: 'box_oscillation' },
  { label: '分析 AMZN 多頭排列', strategy: 'bull_trend' },
  { label: '用情緒週期分析 META', strategy: 'emotion_cycle' },
];
...
const ChatPage: React.FC = () => {
  ...
  // Handle follow-up from report page: ?stock=AAPL&name=Apple&queryId=xxx
  useEffect(() => {
            ...
            if (event.type === 'done') {
              ...
              if (doneEvent.success === false) {
                const parsedStreamError = getParsedApiError(
                  doneEvent.error || doneEvent.content || '大模型呼叫出錯，請檢查 API Key 配置',
                );
  ...
  const getCurrentStage = (steps: ProgressStep[]): string => {
    ...
    if (last.type === 'generating') return last.message || '正在生成最終分析...';
    return '處理中...';
  };
  ...
  const sidebarContent = (
    <>
      <div className="p-3 border-b border-white/5 flex items-center justify-between">
        <span className="text-sm font-medium text-white">歷史對話</span>
        <button
      ...
      <div className="flex-1 overflow-y-auto custom-scrollbar">
        {sessionsLoading ? (
          <div className="p-4 text-center text-xs text-muted">載入中...</div>
        ) : sessions.length === 0 ? (
          <div className="p-4 text-center text-xs text-muted">暫無歷史對話</div>
        ) : (
          sessions.map((s) => (
            <button
              key={s.session_id}
    ...
    <div className="h-screen flex max-w-6xl mx-auto w-full p-4 md:p-6 gap-4">
      ...
      {deleteConfirmId && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50" onClick={() => setDeleteConfirmId(null)}>
          <div className="bg-elevated border border-white/10 rounded-xl p-6 max-w-sm mx-4 shadow-2xl" onClick={(e) => e.stopPropagation()}>
            <h3 className="text-white font-medium mb-2">刪除對話</h3>
            <p className="text-sm text-secondary mb-5">刪除後，該對話將不可恢復，確認刪除嗎？</p>
      ...
      <div className="flex-1 flex flex-col min-w-0">
        <header className="mb-4 flex-shrink-0">
          ...
          <p className="text-secondary text-sm">向 AI 詢問個股分析，獲取基於策略的交易建議與實時決策報告。</p>
        </header>
        ...
        <div className="flex-1 overflow-y-auto p-4 md:p-6 space-y-6 custom-scrollbar relative z-10">
          {messages.length === 0 && !loading ? (
            <div className="h-full flex flex-col items-center justify-center text-center">
              ...
              <h3 className="text-lg font-medium text-white mb-2">開始問股</h3>
              <p className="text-sm text-secondary max-w-sm mb-6">
                輸入「分析 AAPL」或「TSLA 現在能買嗎」，AI 將呼叫實時資料工具為您生成決策報告。
              </p>
        ...
        <div className="p-4 md:p-6 border-t border-white/5 bg-black/20 relative z-20">
          ...
          <div className="flex gap-3 items-end">
            <textarea
              ...
              placeholder="例如：分析 AAPL / TSLA 現在適合買入嗎？ (Enter 傳送, Shift+Enter 換行)"
              disabled={loading}
              rows={1}
