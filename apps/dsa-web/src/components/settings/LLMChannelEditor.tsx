...
export const LLMChannelEditor: React.FC<LLMChannelEditorProps> = ({
  ...
  const handleSave = useCallback(async () => {
    ...
    try {
      ...
      setSaveMessage({ type: 'success', text: '渠道配置已儲存' });
      onSaved();
    } catch (error: unknown) {
      ...
      >
        <div>
          ...
          <p className="mt-0.5 text-xs text-muted">
            {channels.length > 0
              ? `已配置 ${channels.length} 個渠道：${channels.map((c) => c.name).join('、')}`
              : '同時使用多個模型平臺時啟用；只用單個模型可跳過此項'}
          </p>
        </div>
        <span className="text-xs text-muted">{isCollapsed ? '▶ 展開' : '▼ 收起'}</span>
      </button>
        ...
        <div className="mt-4 space-y-3">
          {channels.map((channel, index) => (
            ...
            >
              ...
              {/* Channel name */}
              <div>
                <label className="mb-1 block text-xs text-secondary">渠道名稱</label>
                <input
                  type="text"
              ...
              {/* API Key */}
              <div>
                <label className="mb-1 block text-xs text-secondary">API Key（多個用逗號分隔）</label>
                <div className="flex items-center gap-2">
                  <input
              ...
              {/* Models */}
              <div>
                <label className="mb-1 block text-xs text-secondary">模型列表（逗號分隔）</label>
                <input
                  type="text"
                  className="input-terminal w-full"
                ...
                <p className="mt-1 text-[11px] text-muted">
                  有 Base URL 的渠道無需加 openai/ 字首，系統自動補全
                </p>
              </div>
            </div>
          ))}
          ...
          {hasChanges && (
            <div className="flex items-center gap-3 border-t border-white/8 pt-3">
              ...
              <span className="text-[11px] text-muted">渠道配置獨立儲存，與下方欄位互不影響</span>
            </div>
          )}
