...
export const ImageStockExtractor: React.FC<ImageStockExtractorProps> = ({
    ...
    async (file: File) => {
      ...
      if (!ALLOWED_EXT.includes(ext)) {
        setError(createParsedApiError({
          title: '檔案格式不支援',
          message: '僅支援 JPG、PNG、WebP、GIF 格式。',
          category: 'unknown',
        }));
        return;
      }
  ...
  return (
    <div className="rounded-xl border border-white/8 bg-elevated/40 p-4">
      <p className="mb-2 text-sm font-medium text-white">從圖片新增</p>
      <p className="mb-3 text-xs text-muted">
        上傳自選股截圖，自動識別股票程式碼。需配置 Gemini、Anthropic 或 OpenAI API Key 方可使用。建議人工核對後再合併。
      </p>
      ...
      >
        ...
        />
        {isExtracting ? (
          <span className="text-sm text-secondary">識別中...</span>
        ) : (
          <span className="text-sm text-secondary">
            拖拽或點選上傳圖片（JPG/PNG/WebP，≤5MB）。大圖識別約需 30–60 秒
          </span>
        )}
      </div>
      ...
      {codes.length > 0 ? (
        <div className="space-y-2">
          <p className="rounded-lg border border-amber-500/40 bg-amber-500/10 px-2 py-1.5 text-xs text-amber-400">
            ⚠️ 建議人工逐條核對後再合併，識別結果可能有誤
          </p>
          <p className="text-xs text-secondary">識別結果（可刪除不需要的項）：</p>
