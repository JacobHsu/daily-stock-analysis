...
class TestToolRegistry(unittest.TestCase):
    ...
    def test_execute_success(self):
        ...
        self.registry.register(tool)
        result = self.registry.execute("exec_test", stock_code="AAPL", days=10)
        self.assertEqual(result, {"code": "AAPL", "days": 10})

    def test_execute_default_param(self):
        tool = _make_tool("default_test")
        self.registry.register(tool)
        result = self.registry.execute("default_test", stock_code="AAPL")
        self.assertEqual(result["days"], 30)

    def test_execute_not_found(self):
        with self.assertRaises(KeyError):
            self.registry.execute("not_exist", stock_code="AAPL")
