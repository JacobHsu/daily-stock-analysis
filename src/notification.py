...
class NotificationChannel(Enum):
    ...
    CUSTOM = "custom"      # 自訂 Webhook
    DISCORD = "discord"    # Discord 機器人 (Bot)
    ASTRBOT = "astrbot"
    UNKNOWN = "unknown"    # 未知
    # 以下渠道已停用（保留 enum 值供內部相容，不會被自動偵測）
    PUSHPLUS = "pushplus"
...
class NotificationService(
    ...
    def send_to_context(self, content: str) -> bool:
        """
        向基於訊息上下文的渠道發送訊息（例如釘釘 Stream 會話）
    ...
    def generate_daily_report(
        ...
        report_lines = [
            ...
            "",
            f"> 共分析 **{len(results)}** 支股票 | 報告生成時間：{datetime.now().strftime('%H:%M:%S')}",
            "",
            "---",
            "",
        ]
        ...
        report_lines.extend([
            ...
            "| 指標 | 數值 |",
            "|------|------|",
            f"| 🟢 建議買入/加倉 | **{buy_count}** 支 |",
            f"| 🟡 建議持有/觀望 | **{hold_count}** 支 |",
            f"| 🔴 建議減倉/賣出 | **{sell_count}** 支 |",
    ...
    def generate_dashboard_report(
        ...
        report_lines = [
            f"# 🎯 {report_date} 決策儀表板",
            "",
            f"> 共分析 **{len(results)}** 支股票 | 🟢買入:{buy_count} 🟡觀望:{hold_count} 🔴賣出:{sell_count}",
            "",
        ]
    ...
    def generate_wechat_dashboard(self, results: List[AnalysisResult]) -> str:
        ...
        lines = [
            f"## 🎯 {report_date} 決策儀表板",
            "",
            f"> {len(results)}支股票 | 🟢買入:{buy_count} 🟡觀望:{hold_count} 🔴賣出:{sell_count}",
            "",
        ]
    ...
    def generate_wechat_summary(self, results: List[AnalysisResult]) -> str:
        ...
        lines = [
            ...
            "",
            f"> 共 **{len(results)}** 支 | 🟢買入:{buy_count} 🟡持有:{hold_count} 🔴賣出:{sell_count} | 均分:{avg_score:.0f}",
            "",
        ]
    ...
    def generate_single_stock_report(self, result: AnalysisResult) -> str:
        """
        生成單支股票的分析報告（用於單股推送模式 #55）

        格式精簡但資訊完整，適合每分析完一支股票立即推送
