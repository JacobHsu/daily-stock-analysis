...
class MarketAnalyzer:
    ...
    def get_market_overview(self) -> MarketOverview:
        ...
        Returns:
            MarketOverview: 市場概覽數據物件
        """
        today = datetime.now().strftime('%Y-%m-%d')
        overview = MarketOverview(date=today)

        # 1. 取得主要指數行情（按 region 切換）
    ...
    def _get_main_indices(self) -> List[MarketIndex]:
        ...
        try:
            logger.info("[大盤] 取得主要指數即時行情...")

            # 使用 DataFetcherManager 取得指數行情（按 region 切換）
            ...
            if not indices:
                ...
                logger.info(f"[大盤] 取得 {len(indices)} 個指數行情")

        except Exception as e:
            logger.error(f"[大盤] 取得指數行情失敗: {e}")
    ...
    def _get_market_statistics(self, overview: MarketOverview):
        ...
        try:
            ...
            logger.error(f"[大盤] 取得漲跌統計失敗: {e}")

    def _get_sector_rankings(self, overview: MarketOverview):
        """取得板塊漲跌榜"""
        try:
            logger.info("[大盤] 取得板塊漲跌榜...")
        ...
        try:
            ...
            for query in search_queries:
                ...
                if response and response.results:
                    ...
                    logger.info(f"[大盤] 搜索 '{query}' 取得 {len(response.results)} 條結果")
            
            logger.info(f"[大盤] 共取得 {len(all_news)} 條市場新聞")
    ...
    def generate_market_review(self, overview: MarketOverview, news: List) -> str:
        ...
        if review:
            ...
            logger.warning("[大盤] 大模型回傳為空，使用模板報告")
            return self._generate_template_review(overview, news)
    ...
    def _inject_data_into_review(self, review: str, overview: MarketOverview) -> str:
        ...
        if stats_block:
            review = self._insert_after_section(review, r'###\s*一、市[场場]總?结?總?結?', stats_block)

        # Inject indices table after "### 二、指數點評" section
        if indices_block:
            review = self._insert_after_section(review, r'###\s*二、指[数數]點?评?點?評?', indices_block)

        # Inject sector rankings after "### 四、熱點解讀" section
        if sector_block:
            review = self._insert_after_section(review, r'###\s*四、[热熱][点點]解[读讀]', sector_block)
    ...
    def _build_review_prompt(self, overview: MarketOverview, news: List) -> str:
        ...
        # US-only: build market overview and sector blocks
        stats_block = ""
        sector_block = ""
        if self.profile.has_market_stats:
...
Lagging: {bottom_sectors_text if bottom_sectors_text else "N/A"}"""
        ...
        return f"""你是一位專業的美股市場分析師。請根據以下數據生成一份簡潔的美股大盤復盤報告。請全程使用繁體中文回覆。
...
## {overview.date} 美股大盤復盤

### 一、市場總結
（2-3句話概括今日美股整體表現，包括指數漲跌、成交量變化）

### 二、指數點評
（分析 S&P 500、Nasdaq、Dow Jones 等主要指數走勢）
...
### 七、策略計劃
（給出 risk-on/neutral/risk-off 立場、倉位建議，並給出一個觸發失效條件）
...
"""
    ...
    def _generate_template_review(self, overview: MarketOverview, news: List) -> str:
        ...
        # 按 region 決定是否包含漲跌統計和板塊（部分市場無此數據）
        stats_section = ""
        if self.profile.has_market_stats:
            stats_section = f"""
### 三、漲跌統計
...
| 成交額 | {overview.total_amount:.0f}億 |
"""
        sector_section = ""
