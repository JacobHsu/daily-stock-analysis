...
def setup_logging(
    ...
    配置三層日誌輸出：
    1. 終端：根據 debug 參數或 console_level 設定級別
    2. 常規日誌檔案：INFO 級別，10MB 輪替，保留 5 個備份
    3. 除錯日誌檔案：DEBUG 級別，50MB 輪替，保留 3 個備份

    Args:
        log_prefix: 日誌檔案名前綴（如 "api_server" -> api_server_20240101.log）
        log_dir: 日誌檔案目錄，預設 ./logs
        console_level: 終端日誌級別（可選，優先於 debug 參數）
        debug: 是否啟用除錯模式（終端輸出 DEBUG 級別）
    ...
    # Handler 2: 常規日誌檔案（INFO 級別，10MB 輪替）
